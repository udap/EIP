...
//    internal
//    inSwap
//    permittedSender2(self.swapOffer().target)
//    max128Bytes(note)
    {
        ITradable target;
        uint256 vFrom;
        ...
        ISingularWallet counterOwner = target.toISingular().owner();

        if (counterOwner != self.toISingular().owner()) {
...
//            this.theOwner.received(this, note);
            ...
            require(target.toISingular().owner() == self.previousOwner(),
                "swap() did not change the ownership over to me");
            self.reset();
...
//    }
    ...
    external
    {
        ...
        require(
            coin.toISingular().owner() == self.previousOwner()
            && coin.previousOwner() == self.toISingular().owner()
            ,
