...
contract SingularWalletImpl is ISingularWallet, ReentrancyGuard, Initialized{
    ...
    constructor() Initialized(msg.sender) public {
    }

    function init (address _walletOwner) unconstructed(msg.sender) public {
    ...
    function send(ISingularWallet _to, ISingular _singular, string _senderNote) ownsSingular(_singular) constructed external{
    ...
    function sendNotify(ISingularWallet _to, ISingular _singular, string _senderNote, uint256 _expiry) ownsSingular(_singular) constructed external{
        //send contains approve logic so here emit an approve event
        require(_expiry > now);
    ...
    function approve(ISingularWallet _to, ISingular _singular, string _senderNote, uint256 _expiry ) ownsSingular(_singular) constructed external{
        require(_expiry > now);
        emit SingularReceiverApproved(_to, _singular, now,_senderNote);
        _singular.approveReceiver(_to, _expiry, _senderNote);
    }

    function burn(ISingular _singular, string _burnMsg) constructed external{
    ...
    function agree(ISingular _singular, string _receiverNote) constructed external {
        _singular.accept(_receiverNote);
    }

    //just forward request to singular.reject
    function reject(ISingular _singular, string _receiverNote) constructed external {
        _singular.reject(_receiverNote);
    }
    //=============================reaction===============================================
